...
- SCTP是一个传输层协议，在IP协议上运行，不过他在一个安全的DTLS信道中运行，这个信道又建立在UDP上由于WebRTC支持通过DataChannel api在端对端质传检输任意应用数据，其就依赖SCTP的`可靠性可配置`，`交付次序可配置 `, `面向消息传输方式` `支持流量、拥塞控制`等特性
- 完整的生命周期如下：
